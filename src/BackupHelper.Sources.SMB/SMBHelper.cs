...
internal static class SMBHelper
{
    ...
    public static void ThrowIfStatusNotSuccess(NTStatus status, string operation)
    {
        if (status != NTStatus.STATUS_SUCCESS)
        {
            throw new InvalidOperationException(
                $"SMB operation '{operation}' failed with status: {status}"
            );
        }
    }
    ...
    public static void ThrowIfFileStatusNotFileOpened(
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException(
                $"SMB operation '{operation}' failed to open '{path}' with file status: {fileStatus}"
            );
        }
    }

    public static void ThrowIfFileStatusNotFileCreated(
        FileStatus fileStatus,
        string path,
        string operation
    )
    {
        ...
        {
            throw new InvalidOperationException(
                $"SMB operation '{operation}' failed to create '{path}' with file status: {fileStatus}"
            );
        }
    }
}
